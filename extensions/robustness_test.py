...
Usage:
    python extensions/robustness_test.py \
        --model_path nnunet_results/Dataset001_TCGALGG/nnUNetTrainer__nnUNetPlans__2d/fold_best \
    ...
    for patient in sorted(os.listdir(datapath)):
        ...
        for f in sorted(os.listdir(ppath)):
            if f.endswith('.tif') and '_mask' not in f:
                ...
                if os.path.exists(os.path.join(ppath, mask_f)):
                    mask = np.array(Image.open(os.path.join(ppath, mask_f)).convert('L'))
                    if mask.max() > 0:  # only tumor slices
                        pairs.append((os.path.join(ppath, f), os.path.join(ppath, mask_f)))
        if len(pairs) >= n:
            break
    return pairs[:n]
...
# ── Simulate without a trained model: use simple threshold as proxy ───────────
# NOTE: Replace this with actual nnU-Net inference once weights are available.
# This section provides the degradation framework and SSIM/PSNR quality metrics.
def mock_segment(img):
    # Naive Otsu-like threshold as segmentation proxy
...
for deg_name, deg_fn in DEGRADATIONS.items():
    ...
    for flair_path, mask_path in tqdm(pairs, desc=deg_name, leave=False):
        ...
        # Image quality: SSIM and PSNR vs clean
    ...
    print(f"  {deg_name:<20}: SSIM={results[deg_name]['ssim_mean']:.3f}  "
          f"PSNR={results[deg_name]['psnr_mean']:.1f}dB  "
          f"Dice={results[deg_name]['dice_mean']:.3f}")
...
colors = ['#4CAF50' if n == 'clean' else '#F44336' if d < 0.6 else '#FF9800'
